...
> [!info] 문서 구성
>
> - 본문은 복습에 필요한 핵심 개념과 모델의 발전 흐름을 간결하게 정리한다.
> - `질문` 콜아웃에는 헷갈리기 쉬운 원리와 추가 설명을 담는다.
> - 복잡한 수식과 세부 비교는 핵심 흐름을 이해한 뒤 참고할 수 있도록 분리한다.

이 문서는 자연어를 수치로 표현하는 방법에서 출발해, 순서와 문맥을 처리하는 모델이 발전해 온 과정을 설명한다.
...
각 단계는 앞선 구조의 한계를 해결하면서 등장했다. 원-핫 인코딩은 의미적 유사성을 표현하지 못해 임베딩으로 확장되었고, RNN은 순서를 처리했지만 장기 의존성과 병렬화에 한계가 있었다. Attention과 Transformer는 정보 전달 경로를 단축하고 병렬 처리를 가능하게 했으며, 대규모 사전학습과 결합해 다양한 LLM 아키텍처로 발전했다.
...
- 각 단어를 서로 구별되는 기호로 취급하고, **한 원소만 1이고 나머지는 0**인 벡터로 표현한다.
- 벡터 차원은 단어 사전의 크기와 같다. 예: 음성 데이터 약 2만, PTB 약 5만, Google 1T 코퍼스 약 1,300만 차원
...
- **분포가설**: 비슷한 문맥에 등장하는 단어는 비슷한 의미를 갖는다는 가정
...
- 2013년에 발표된 대표적인 워드 임베딩 기법으로, 주변 단어 예측 과제를 통해 단어 벡터를 학습한다.
- 핵심 아이디어: **각 단어와 그 주변 단어들 간의 관계를 예측**
- 두 가지 알고리즘 존재: Skip-gram(SG), CBOW

#### Skip-gram (SG)

- **중심 단어로 주변 단어를 예측**하는 방식. 기본 구조에서는 윈도우 안에 포함되는 문맥 단어를 각각 예측한다.
...
- 선형 회귀와 일반적인 MLP는 고정된 크기의 입력을 한 번에 처리하며, 순서와 가변 길이를 별도로 모델링하지 않는다.
- 위치별 입력을 고정하거나 위치 정보를 추가하면 처리할 수는 있지만, 길이 변화와 파라미터 재사용을 자연스럽게 다루기 어렵다.
- 이러한 특성을 구조적으로 처리하기 위해 RNN, LSTM, Transformer와 같은 **Sequential Model**을 사용한다.
...
- RNN을 시간축으로 펼치면 동일한 셀이 각 시점에 반복 적용된 계산 그래프로 나타난다. 서로 다른 레이어가 아니라 **같은 파라미터를 공유하는 시점별 연산**이다.
...
- 목표: 한국어 문장 $x$가 주어졌을 때 조건부 확률이 가장 높은 영어 문장 $y$를 찾는다.
...
- 이때 사용되는 신경망 구조를 **sequence-to-sequence (Seq2Seq)**라 하며, **두 개의 RNN(LSTM)**으로 이루어짐
    - 두 개의 RNN = **인코더 셀 1개 + 디코더 셀 1개**. 각 셀은 자기 구간(인코더/디코더) 안에서만 시점마다 재사용되며, 인코더와 디코더는 서로 다른 가중치를 씀(공유 안 함)
...
2. **Bottleneck Problem 완화**: 디코더가 인코더의 모든 hidden state에 직접 접근할 수 있음
...
- Seq2Seq attention에서 디코더 hidden state는 **Query**, 인코더 hidden state는 유사도 계산에 쓰이는 **Key**이자 가중합할 정보인 **Value**로 볼 수 있다.
- 이 관점에서 Attention 과정을 일반화하면:
    1. Query와 Key 사이 유사도 점수(score) 계산(예: dot product, multiplicative, additive)
    2. Softmax를 통해 확률 분포(attention distribution) 얻기
    3. 이 분포를 가중치로 Value를 합산해 context vector(attention output) 생성
...
Seq2Seq Attention은 디코더가 인코더의 모든 시점에 직접 접근할 수 있게 했다. 여기서 한 단계 더 나아가 같은 시퀀스 내부의 모든 토큰이 서로를 직접 참조하도록 만든 것이 Self-Attention이다. Transformer는 이 메커니즘을 중심으로 RNN의 순환 구조를 제거해 장거리 정보 전달과 병렬 학습을 개선했다.
...
- RNN은 정보를 **hidden state를 통해 순차적으로 전달**하며 시점 간 의존성(sequential dependency)을 형성한다.
- 반면 Attention은 필요한 순간, **입력 전체에서 직접 정보를 전달**할 수 있음
- 이 관찰에서 순환 구조 없이 Attention만으로 시퀀스를 처리할 수 있는지에 대한 연구가 출발했다.

#### RNN의 한계점 1 — 장기 의존성

- RNN은 왼쪽에서 오른쪽으로 순차적으로 전개되어, **멀리 떨어진 단어 쌍이 상호작용하려면 시퀀스 길이만큼의 단계**를 거쳐야 함
- 단계가 길어질수록 기울기 소실/폭발 문제가 심해져 장기 의존성 학습이 어려움(1.3 참고)
- RNN은 멀리 떨어진 정보를 여러 시점을 거쳐 전달해야 하므로, 거리가 긴 의존성을 학습하기 어렵다.
...
- 각 시점의 계산이 이전 시점의 결과에 의존하므로 **시간축 방향의 병렬화가 제한된다.**
- 이에 따라 GPU의 병렬 연산 능력을 충분히 활용하기 어려워 대규모 데이터 학습의 효율이 낮다.

#### Self-Attention의 등장

- Attention은 각 단어의 표현을 **query**로 두고, key와의 관련도에 따라 **value 집합**을 가중합한다(2.3 Query·Key·Value 참고).
- 이 Attention 메커니즘을 **encoder-decoder 간이 아니라, 한 문장 내부에서 적용**하면? → **Self-Attention**

**Self-Attention의 장점**

1. 한 층 안의 모든 토큰 표현을 병렬로 계산할 수 있다.
2. 최대 상호작용 거리 = **O(1)**: 모든 토큰이 한 층에서 직접 상호작용한다.
...
2. **위치별 비선형 변환의 필요**: attention 가중치는 softmax로 비선형적으로 계산되지만, 최종 출력은 Value의 가중합이다. 각 위치의 표현을 더 복잡하게 변환하려면 별도의 Feed-Forward Network가 필요하다.
3. **미래 참조 문제(자기회귀 디코더)**: 기본 Self-Attention은 시퀀스 전체를 참조한다. 다음 토큰 예측에서는 현재 위치 이후의 정답 토큰을 보면 안 되므로 미래 위치를 차단해야 한다.
...
- Self-Attention은 단어 사이에서 정보를 결합하는 역할을 맡고, 각 위치의 표현을 비선형적으로 가공하는 능력은 제한적이다.
- 각 단어 출력 벡터에 동일한 Feed-Forward Network(Linear → 활성화 함수 → Linear)를 적용해 표현을 비선형적으로 확장한다.
...
- 해결책: Query·Key 차원 $d_k$의 제곱근으로 내적을 나누어 분산을 조절한다.
...
- 각 토큰 표현의 feature 차원을 기준으로 평균과 분산을 정규화해 학습을 안정화한다.
...
- Encoder의 Self-Attention에는 causal mask를 사용하지 않는다. 또한 원래 Transformer의 Decoder 블록에는 Masked Self-Attention 외에 Encoder 출력을 참조하는 Cross-Attention sublayer가 추가된다는 점도 다르다.
...
- Transformer는 Neural Machine Translation task에서 당시 최고 수준의 BLEU 점수를 기록했으며, 기존 주요 모델보다 적은 학습 연산량으로 높은 성능을 보였다.
...
Transformer는 대규모 텍스트를 병렬로 학습하기에 적합하다. 이 계산 구조가 사전학습과 결합하면서 하나의 모델이 언어의 일반 패턴을 먼저 학습한 뒤 여러 태스크에 적응하는 방식이 표준이 되었다. Transformer 블록의 사용 방식에 따라 BERT와 같은 Encoder 모델, T5와 같은 Encoder-Decoder 모델, GPT와 같은 Decoder 모델로 분화한다.
...
- 언어 모델은 웹 문서·책·뉴스 등의 방대한 텍스트에서 입력 자체로 학습 목표를 구성하는 **자기지도학습(self-supervised learning)**을 수행한다. 예: 다음 토큰 예측, 마스킹된 토큰 복원
...
- Fine-tuning은 사전학습된 backbone과 파라미터를 출발점으로 재사용한다. 태스크에 따라 분류기와 같은 출력층을 추가할 수 있지만, 사전학습으로 얻은 핵심 표현은 그대로 이어받는다.
...
- Encoder 모델은 입력 전체의 양방향 문맥을 활용하므로, 왼쪽 문맥만 사용하는 다음 토큰 예측 대신 마스킹된 위치의 앞뒤 문맥을 함께 사용하는 학습 목표가 적합하다.
...
- 입력 토큰의 **15%를 무작위로 선택**
- 선택된 토큰은 세 방식으로 처리한다: **[MASK] 치환 80%**, **무작위 토큰 치환 10%**, **원래 토큰 유지 10%**
...
- 이를 보완하기 위해 **Chain-of-Thought(CoT) prompting**이 등장했다. 중간 풀이 단계를 텍스트로 생성한 뒤 최종 답을 도출하도록 유도하는 prompting 기법이다.
